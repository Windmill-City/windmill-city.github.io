...
这意味着使用 `position: fix | absolute;` 将元素固定的方法是行不通, 或者说是不够优雅的

所以应该使用 `position: sticky` 配合 `top | bottom | left |right` 来进行元素的固定

还要注意的是, `Flexbox` 默认的 `align-items` 属性是 `stretch`, 这会导致元素在主轴上拉伸

![Stretched](/assets/Stretched.png)

箭头下方的空白是拉伸的结果, 观察调试工具 `Computed` 中的 `height = 3032.02px`

这意味着这个元素是一直延伸到 `DOM` 底部, 产生 `position: sticky` 不生效的假象
